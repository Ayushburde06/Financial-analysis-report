...
The health response should report `"status": "ok"` when the required Azure provider variables are configured. The EC2 security group should allow the application port only as needed; put the service behind HTTPS/reverse proxy infrastructure for a public deployment.
